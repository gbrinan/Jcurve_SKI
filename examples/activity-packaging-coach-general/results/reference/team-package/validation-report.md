...
`Sourcing Operations Lab` 패키지는 3개 독립 AI 작업이 하나의 도달 가능한 체인을 이루고 L0–L4, 로컬 실행 E2E, Activity 실행 목업 12상태, 발표자료 27화면 검사를 통과했으므로 `team agent`다. 다만 운영 `sourcing-policy.csv` 열 스키마는 승인 입력에 없어서 `(미확인)`으로 유지했으며, 실제 운영 연결 전 구매담당자가 확인해야 한다. 합성 fixture 실행은 가능하지만 production activation은 이 스키마 확인 전까지 보류한다.
...
| Local runner | PASS | `node run.mjs`가 출력 4종과 `trace.json`을 재생성. `evidence/run-e2e.mjs`에서 원본 ID 3개, 정상·임계·파싱실패 분기, 사람 정지 2개, 외부 행동 0개를 독립 확인. |

## Activity execution mockup QA

- Status: `PASS — 12/12`
- Surface: `agent-mockup.html`, recorded source `data/output/trace.json`
- Browser: Microsoft Edge Chromium, Playwright headless, reduced-motion
- Viewports and states: `375×667`, `768×720`, `1280×900` × `rest`, `running`, `hold-intermediate`, `hold-final`
- Fit and safety: document horizontal overflow 1px 이하, visible card와 fixed control dock viewport 내부, 외부 행동 버튼 전부 disabled.
- Interaction: 실행, 단계 진행, 중간 사람 확인 뒤 재개, 최종 사람 확인 정지, R 키 reset을 실제 DOM에서 확인.
