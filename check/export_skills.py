...
def export_team_skill(pack, sk, facts, dest):
    ...
    if facts["one"]:
        body += [facts["one"], ""]
    body += ["## 실행 규칙", "",
             "0. **데이터 정본** — 현재 작업 폴더에 `data/`가 있으면 **그것만** 읽는다.",
             "   이 스킬 폴더 안의 `data/`는 작업 폴더에 data가 없을 때 쓰는 예비 사본이다.",
             "   두 곳의 값이 다르면 작업 폴더 쪽이 정본이다.",
    ...
    if loops:
        ...
        body += ["   되돌아갈 때 **원본 `data/`를 고치지 않는다.** 원본은 사람이 넣은 입력이다.",
                 "   사용자가 알려준 조정 내용을 `out/`의 결과에만 반영해 다시 계산하고,",
                 "   무엇을 조정한 것으로 보고 다시 돌렸는지 회차와 함께 적는다.",
                 "   조정 내용을 사용자가 말해주지 않았으면 추측하지 말고 묻는다.",
                 "   같은 자리를 5회 넘게 돌면 멈추고 사람에게 넘긴다 — 무한히 돌지 않는다.", ""]
    body += [f"5. ⛔ **정지 지점: {halt}**",
