#!/usr/bin/env bash
# 상류 코치 산출물 → 스킬 → 게이트 → 통합 점검기 전 구간 테스트
#
...
echo "═══ 1. 상류 산출물 → 스킬 변환 ═══"
python3 check/adapt_upstream.py examples/upstream/wireframe \
        examples/upstream/atf/ATF보고서_전황업데이트.html "$PACK" || exit 1

echo
echo "═══ 2. 계약 게이트 (상류 그대로 = 정보 부족이므로 RED가 정답) ═══"
python3 check/check_contract.py "$PACK" >/dev/null 2>&1
code=$?
if [ $code -eq 2 ]; then
...
python3 check/fixtures_session6.py "$PACK" || exit 1

echo
echo "═══ 4. 게이트 → 통합 점검기 (여기서 전체 통과해야 함) ═══"
