...
**실습 시나리오 — 멈춤과 재개까지 해보십시오.** 데이터가 있는 폴더에서:

```
분기-반기-결산 팀 에이전트를 실행해줘
```

1. 에이전트가 체인대로 돌다가 매출채권 차이 120,000,000을 발견하고
   **차이분석조정에서 멈춥니다** — 조정 금액을 정하지 않고 목록·근거만 내놓습니다.
2. 사람이 확인했다고 알려주면 계속 갑니다:

```
차이는 기간귀속 차이로 확인됐다. 조정 확정. 계속 진행해
```

3. 계정검증대사로 **루프백**해 2회차에 차이 0을 확인하고, 결산마감 점검표를
   만든 뒤 **다시 멈춥니다.** 원장 Lock·승인은 에이전트가 절대 대신하지 않습니다.

`data/결산데이터.md`를 고치면(위 4절의 값 바꾸기) 에이전트의 흐름도 실제로
달라집니다 — 차이가 0이면 루프백 없이 바로 결산마감에서 멈춥니다.
