...
> **출력**: `agent-plan.md` · 폴더 트리 · `CONTRACT.md` · `AGENTS.md` · 동작 테스트 결과 · **`agent-mockup.html`**(실행 확인 화면).
...
| 파일 | 쓰임 |
|---|---|
| `result_reference.html` | 설명 HTML의 서식 정본. `<style>`·`<script>`·구조를 복제하고 데이터만 갈아 끼운다 |
| `templates/agent-plan.md` | 기획서 8개 절 골격 |
| `templates/AGENTS.md` | 페르소나·행동 규칙 골격 |
| `templates/CONTRACT.md` | 계약 파일 골격 + 위반 시 처리 표 |
| `activity-mockup-template.html` | 실행 확인 화면(agent-mockup.html)의 서식 정본 |

템플릿은 다른 팀(구매기획팀) 예시다. **골격만 가져오고 내용은 우리 팀 것으로 채운다.**

## agent-mockup.html — 실행 확인 화면 생성 규칙

동작 테스트가 끝난 뒤, 팀이 결과를 재생하며 확인할 수 있는 화면을 만든다.

1. 같은 폴더의 `activity-mockup-template.html`을 **통째로 복제**한다. CSS·JS·구조를 새로 짓지 않는다
2. `trace-data` JSON만 이 팀의 실행 기록으로 갈아 끼운다 — 단계(`steps`)는 체인 순서 그대로,
   `kind`는 자동=system / 검토 갈림길=review / 사람 정지=human, 사람 정지에는 `hold`(intermediate|final)를 넣는다
3. 헤더·입력 카드·용어의 도메인 문구(업무명·Lv 경로·입력 설명·외부 행동 버튼 이름)를 이 팀 것으로 바꾼다
4. 수치는 실제 실행 산출물(out/)에서 가져온다. 지어내지 않는다
5. **외부 행동 버튼은 반드시 `disabled`로 둔다** — 목업은 재생이지 실행이 아니다
6. QA: 375·768·1280 세 폭 × 4상태(대기 rest / 실행 running / 중간 정지 hold-intermediate / 최종 정지 hold-final)를 확인한다

완성 예시: `examples/sample/balance-agent/agent-mockup.html`
